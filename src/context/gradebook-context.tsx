...
const GradebookContext = createContext< GradebookContextValue | undefined >(
	undefined
);
...
function gradebookReducer(
	...
	switch ( action.type ) {
		...
		case SET_GRADEBOOK: {
			const students = Array.isArray( action.payload.students )
		...
		case REMOVE_STUDENT:
			return {
				...
				cells: state.cells.filter( ( c ) => c.uid !== action.payload ),
			};
		...
		case REMOVE_ASSIGNMENT:
			return {
				...
				cells: state.cells.filter( ( c ) => c.amid !== action.payload ),
			};
...
export function GradebookProvider( { children }: GradebookProviderProps ) {
