...
src/
  ...
  App.js                       Root component (routes by user role)
  api/                         API client modules (@wordpress/api-fetch)
  context/                     React Context + useReducer state management
  hooks/                       Custom hooks (useGradebook, useSorting)
  components/
    InstructorApp.js            Instructor root
    StudentApp.js               Student root
    CourseList/                  Course table with CRUD actions
    GradebookTable/             Main spreadsheet (inline editing, sort, filter)
    StudentCourseList/           Student course list (read-only)
    StudentGradebook/            Student gradebook (read-only)
    charts/                      Recharts pie + line charts
    modals/                      Modal dialogs (forms, confirmations, stats)
  styles/
    gradebook.css                Custom styles (WordPress admin native classes)
build/                           Generated by @wordpress/scripts (not committed)
```

The frontend is a React single-page application. All data flows through the WordPress REST API (`/wp-json/an-gradebook/v1/`) secured with nonce verification and capability checks. React and ReactDOM are provided by WordPress core via `wp-element` — they are not bundled into the plugin.
...
- **Deploy:** On tag push (or manual `workflow_dispatch` with "deploy" confirmation), the plugin is built and deployed to WordPress.org via SVN.
