(function($, Backbone, _, ANGradeBook) {
    'use strict';
    ANGradeBook.Views.CourseView = Backbone.View.extend({
        tagName: 'tr',
        events: {
        ...
        editCourse: function() {
            var view = new ANGradeBook.Views.EditCourseView({model: this.course, options: this.options});
            return false;
        },
        render: function() {
        ...
        selectCourse: function(ev) {
            var x = this.students.findWhere({selected: true});
            x && x.set({selected: false});
            var y = this.assignments.findWhere({selected: true});
            y && y.set({selected: false});
            if (this.course.get('selected')) {
                this.course.set({selected: false});
            } else {
                var x = this.courses.findWhere({selected: true});
                x && x.set({selected: false});
                this.course.set({selected: true});
            }
        },
