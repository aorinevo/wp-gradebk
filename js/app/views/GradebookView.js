(function($, Backbone, _, ANGradeBook) {
    'use strict';
    ANGradeBook.Views.GradebookView = Backbone.View.extend({
        initialize: function(options) {
            ...
            this.courseGradebook = new ANGradeBook.Models.CourseGradebook(this.course.attributes);
        ...
        selectAllStudents: function(ev) {
            ...
            if (_selected) {
                _.each(this.students.models, function(student) {
            ...
            switch (this.sort_key) {
                case 'cell':
                    _.each(this.sort_column, function(cell) {
                        var view = new ANGradeBook.Views.StudentView({
                            model: self.students.get(cell.get('uid')), options: self.options
                        });
                        $('#students').append(view.render().el);
                    });
                    var y = self.assignments.where({gbid: parseInt(this.course.get('id'))});
                    y = _.sortBy(y, function(assign) { return assign.get('assign_order'); });
                    _.each(y, function(assignment) {
                        var view = new ANGradeBook.Views.AssignmentView({model: assignment, options: self.options});
                        $('#students-header tr').append(view.render().el);
                    });
                    break;
                case 'lastname':
                    _.each(this.sort_column.models, function(student) {
                        var view = new ANGradeBook.Views.StudentView({model: student, options: self.options});
                        $('#students').append(view.render().el);
                    });
                    var y = self.assignments.where({gbid: parseInt(this.course.get('id'))});
                    y = _.sortBy(y, function(assign) { return assign.get('assign_order'); });
        ...
        filterAssignments: function() {
            ...
            var _toHide = this.assignments.filter(function(assign) {
                return assign.get('assign_category') != _x;
            });
            var _toShow = this.assignments.filter(function(assign) {
                return assign.get('assign_category') === _x;
            });
            if (_x === "-1") {
        ...
        addAssignment: function(ev) {
            var view = new ANGradeBook.Views.EditAssignmentView({options: this.options});
        },
        addStudent: function(ev) {
            var view = new ANGradeBook.Views.EditStudentView({options: this.options});
        },
