(function($, Backbone, _, ANGradeBook) {
    'use strict';
    ANGradeBook.Views.StudentGradebookView = Backbone.View.extend({
        initialize: function(options) {
            ...
            this.courseGradebook = new ANGradeBook.Models.StudentCourseGradebook(this.course.attributes);
            ...
            switch (this.sort_key) {
                case 'cell':
                    _.each(this.sort_column, function(cell) {
                        var view = new ANGradeBook.Views.StudentStudentView({
                    ...
                    _.each(y, function(assignment) {
                        var view = new ANGradeBook.Views.StudentAssignmentView({model: assignment, options: self.options});
                        $('#students-header tr').append(view.render().el);
                    });
                    break;
                case 'lastname':
                    _.each(this.sort_column.models, function(student) {
                        var view = new ANGradeBook.Views.StudentStudentView({model: student, options: self.options});
                        $('#students').append(view.render().el);
                    });
                    var y = self.assignments.where({gbid: parseInt(this.course.get('id'))});
                    y = _.sortBy(y, function(assign) { return assign.get('assign_order'); });
        ...
        filterAssignments: function() {
            ...
            if (_x === "-1") {
                ...
                _.each(_toHide, function(assign) { assign.set({visibility: false}); });
                _.each(_toShow, function(assign) { assign.set({visibility: true}); });
            }
        },
