export default function CourseRow( {
	...
	return (
		<tr className={ isSelected ? 'an-gb-row-selected' : '' }>
			<td className="title column-title has-row-actions column-primary" data-colname="Course">
				<strong>
					<button
						type="button"
				...
				<div className="row-actions">
					...
					<span className="export">
						<button type="button" className="button-link" onClick={ onExport }>
							Export CSV
						</button>
						{ ' | ' }
					</span>
					<span className="trash">
						<button type="button" className="button-link-delete" onClick={ onDelete }>
							Delete
						</button>
					</span>
				</div>
