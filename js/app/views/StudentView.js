(function($, Backbone, _, ANGradeBook) {
    'use strict';
    ANGradeBook.Views.StudentView = Backbone.View.extend({
        tagName: 'tr',
        events: {
        ...
        render: function() {
            ...
            _.each(x, function(cell) {
                var view = new ANGradeBook.Views.CellView({model: cell, options: self.options});
        ...
        editStudent: function(ev) {
            ev.preventDefault();
            var view = new ANGradeBook.Views.EditStudentView({model: this.model, course: this.course, options: this.options});
        },
        deleteStudent: function(ev) {
            ev.preventDefault();
            var view = new ANGradeBook.Views.DeleteStudentView({model: this.model, options: this.options});
        },
        close: function(ev) {
